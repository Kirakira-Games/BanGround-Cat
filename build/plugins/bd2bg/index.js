...
function apply(ctx, options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const kiraconf = util_1.getKiraConf();
        ...
        command.subcommand("convert <id: number>", "转换 Bestdori 谱面到 BanGround 的谱面并打包")
            ...
            try {
                if (id === null || id === undefined)
                    return util_1.withQuoteBySession(argv, "使用示例：kirapack 128");
                if (typeof id !== "number" || id <= 0)
                    return util_1.withQuoteBySession(argv, "您提供的ID无效，请确认后再试。");
                const timer = setTimeout(() => {
                    var _a;
                    (_a = argv.session) === null || _a === void 0 ? void 0 : _a.send(util_1.withQuoteBySession(argv, `正在为您查询 ID 为 ${id} 的谱面，请稍候……`)).catch(console.error);
                }, 3000);
                const chart = yield BestdoriUtil_1.mixLookupCharts(id);
                clearTimeout(timer);
                if (!chart)
                    return util_1.withQuoteBySession(argv, `ID 为 ${id} 的谱面不存在，请检查后再试。`);
                if (chart.official) {
                    return "是官谱啊喵？咕咕咕咕咕咕";
                    if (!fs_1.default.existsSync(index_1.__data_dir + `/kirapack/official_${id}.kirapack`)) {
                        (_a = argv.session) === null || _a === void 0 ? void 0 : _a.send(util_1.withQuoteBySession(argv, `正在为您打包生成 ${BestdoriUtil_1.getByLocale(chart.official.title, BestdoriUtil_1.BandoriServer.cn)}，请稍候……`)).catch(console.error);
                    }
                    (_b = argv.session) === null || _b === void 0 ? void 0 : _b.send(util_1.withQuoteBySession(argv, `↓↓↓复制下方链接到浏览器以下载 [${id}]${BestdoriUtil_1.getByLocale(chart.official.title, BestdoriUtil_1.BandoriServer.cn)}`)).catch(console.error);
                }
                if (chart.community) {
                    if (!fs_1.default.existsSync(index_1.__data_dir + `/kirapack/community_${id}.kirapack`)) {
                        (_c = argv.session) === null || _c === void 0 ? void 0 : _c.send(util_1.withQuoteBySession(argv, `正在为您打包生成 ${chart.community.title}，请稍候……`)).catch(console.error);
                        {
                    ...
                    yield ((_d = argv.session) === null || _d === void 0 ? void 0 : _d.send(util_1.withQuoteBySession(argv, `↓↓↓复制下方链接到浏览器以下载 [${id}]${chart.community.title}`)).catch(console.error));
                    yield ((_e = argv.session) === null || _e === void 0 ? void 0 : _e.send(util_1.withQuoteBySession(argv, `${kiraconf.kirapack.baseURL}${kiraconf.kirapack.baseURL.endsWith("/") ? "" : "/"}community_${id}.kirapack`)).catch(console.error));
                }
                return "";
            }
            catch (e) {
